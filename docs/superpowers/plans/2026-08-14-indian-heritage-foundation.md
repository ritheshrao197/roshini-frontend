...
- **`package.json`'s version field will change on every commit regardless of what a task touches.** `client-next/.git/hooks/pre-commit` unconditionally bumps the patch version and stages it. This is expected repo behavior, not a scope violation — do not flag it as one in any task review.
