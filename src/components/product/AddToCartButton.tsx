...
export default function AddToCartButton({
  ...
  if (quantity > 0) {
    return (
      <div className="flex-1 flex items-center bg-[#F5E9DA] rounded-xl border border-[#E8D5BC] overflow-hidden justify-between">
        <button
          type="button"
        ...
        <span className="px-2 text-xs sm:text-sm font-bold text-[#6B3E26] text-center select-none uppercase tracking-wider">
          {quantity} in Cart
        </span>
        <button
          type="button"
  ...
  return (
    <button
      onClick={handleAdd}
      className="flex-1 flex items-center justify-center gap-2 px-4 sm:px-6 py-2.5 sm:py-3 rounded-xl font-bold text-xs sm:text-sm uppercase tracking-wider transition-all duration-200 cursor-pointer"
      style={{
        background: "#6B3E26",
        color: "#F5E9DA",
        boxShadow: "0 2px 10px rgba(107,62,38,0.2)",
      }}
    >
